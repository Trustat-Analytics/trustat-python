        ...
        side_effect=[
            resp(
                200,
                {
                    "status": "ok",
                    "response": page_body(
                        "channels", [{"channel_id": 1}, {"channel_id": 2}], next_cursor="c1", total=5
                    ),
                },
            ),
            resp(
                200,
                {
                    "status": "ok",
                    "response": page_body(
                        "channels", [{"channel_id": 3}, {"channel_id": 4}], next_cursor="c2", total=5
                    ),
                },
            ),
            resp(
                200, {"status": "ok", "response": page_body("channels", [{"channel_id": 5}], next_cursor=None, total=5)}
            ),
        ]
    )
...
def test_does_not_use_total_to_bound_loop(client):
    ...
    respx.get(f"{BASE}/public/v1/posts/search").mock(
        return_value=resp(
            200, {"status": "ok", "response": page_body("posts", [{"message_id": 1}], next_cursor=None, total=100)}
        )
    )
